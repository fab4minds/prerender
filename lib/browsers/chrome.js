...
chrome._getCurrentlyUnusedTab = function() {
	let tab = this.tabs.getValue().find(tab => !tab.prerender || tab.prerender.responseSent);
	if(tab) {
		tab.prerender.responseSent = false;
	}
	return tab;
	...
	if(!this._initializedQueue) {
		...
		return Promise.all(promises).then(() => {
			this.tabs.pipe(switchMap(tabs => {
				return merge(...tabs.map(tab => tab.prerender.o_responseSent.pipe(skip(1), distinctUntilChanged(), filter(responseSent => responseSent), mapTo(tab)))).pipe(tap(unusedTab => {
					...
					if(this.options.numRequestsAutoRestart && unusedTab.prerender.numPagesFinished >= this.options.numRequestsAutoRestart) {
						...
						unusedTab.prerender.responseSent = false;
					...
					} else if(this._waitingCallbacks.length) {
						this._waitingCallbacks[0](unusedTab);
