...
const {first, mapTo, merge, BehaviorSubject, filter, skip, distinctUntilChanged, switchMap, tap} = require("rxjs");
...
chrome.getNewTab = function(prerenderObj) {
	return new Promise((resolve, reject) => {
		...
		connectToBrowser(this.webSocketDebuggerURL, this.options.browserDebuggingPort)
			...
			tab.browser = browser;
			if(prerenderObj) {
				tab.prerender = prerenderObj;
			} else {
				tab.prerender = {};
				tab.prerender._responseSent = new BehaviorSubject(true);
				Object.defineProperty(tab.prerender, "responseSent", {
					get: function () {
						return this._responseSent.getValue();
					},
					set: function (value) {
						this._responseSent.next(value);
					}
				});
				tab.prerender.o_responseSent = tab.prerender._responseSent.asObservable();
			}
			tab.prerender.numPagesFinished = 0;
			this._resetPrerenderVariables(tab);
			return this.setUpEvents(tab).then(() => {
				this.loadUrlThenWaitForPageLoadEvent(tab, this.options.baseUrl).then(() => {
					tab.prerender.responseSent = true;
					resolve(tab);
				});
			});
		}).catch((err) => {
...
chrome._resetPrerenderVariables = function(tab) {
	tab.prerender.responseSent = false;
	tab.prerender.errors = [];
	tab.prerender.requests = {};
	tab.prerender.numRequestsInFlight = 0;
	this._resetPageLoadInfo(tab);
}

chrome._getCurrentlyUnusedTab = function() {
	let tab = this.tabs.getValue().find(tab => !tab.prerender || (tab.prerender.responseSent && !tab.isDisposing));
	if(tab) {
		tab.prerender.responseSent = false;
	}
	return tab;
...
chrome.getUnusedTab = async function() {
	let unusedTab = this._getCurrentlyUnusedTab();
	if(!unusedTab) {
		return new Promise(resolve => {
			this._waitingCallbacks.push(resolve);
		});
	}
	return unusedTab;
}

chrome._initTabQueue = async function() {
	if(!this._initializedQueue) {
		this._initializedQueue = true;
		if(!this._waitingCallbacks) {
			this._waitingCallbacks = [];
		}
		let promises = new Array(this.options.numTabs).fill(null).map(() => {
			return this.getNewTab().then(tab => this.tabs.getValue().push(tab));
		});
		return Promise.all(promises).then(() => {
			this.tabs.pipe(switchMap(tabs => {
				return merge(
					...tabs.map(tab => tab.prerender.o_responseSent.pipe(
							skip(1),
							distinctUntilChanged(),
							filter(responseSent => responseSent && !tab.isDisposing),
							mapTo(tab)
						)
					)
				).pipe(tap(unusedTab => {
					unusedTab.prerender.numPagesFinished++;
					if(this.options.numRequestsAutoRestart && unusedTab.prerender.numPagesFinished >= this.options.numRequestsAutoRestart) {
						let tabId = unusedTab.target;
						util.log("Restarting tab as it has already been used for " + unusedTab.prerender.numPagesFinished + " pages (tabId: " + tabId + ")");
						unusedTab.isDisposing = true;
						this.closeTabAndOpenNew(unusedTab).then(newTab => {
							let allTabs = [...this.tabs.getValue()];
							let indexOfOldTab = allTabs.indexOf(unusedTab);
							if(indexOfOldTab > -1) {
								allTabs.splice(indexOfOldTab, 1, newTab);
								util.log("Tab successfully restarted (old tabId: " + tabId + "; new tabId: " + newTab.target + ")");
								this.tabs.next(allTabs);
								this._executeWaitingCallback(newTab);
							} else {
								util.log("warning: created new tab, but couldn´t find old tab to delete(old tabId: " + tabId + ") - closing new tab ...");
								this.closeTab(newTab);
							}
						});
					} else {
						this._executeWaitingCallback(unusedTab);
					}
				}));
			})).subscribe();
		});
...
chrome._executeWaitingCallback = function(tab) {
	if(this._waitingCallbacks.length) {
		this._waitingCallbacks[0](tab);
		this._waitingCallbacks.splice(0, 1);
	}
}

chrome.closeTabAndOpenNew = async function (tab) {
	await this.closeTab(tab);
	return await this.getNewTab(tab.prerender);
}
...
chrome.setUpEvents = async function (tab) {
	...
	Network.responseReceived((params) => {
		...
		if (params.requestId == tab.prerender.initialRequestId && !tab.prerender.receivedRedirect) {
			...
			if(!tab.prerender.statusCode || tab.prerender.statusCode == 200) {
		...
		} else if(this._isErrorStatusCode(params.response.status) && params.type == "XHR") {
			let parsedUrl = params.response.url && url.parse(params.response.url);
			if(!parsedUrl || !parsedUrl.path || parsedUrl.path != '/ACM/api/auth/currenttoken' || params.response.status != 401) { // ignore 'unauthorized' when calling currenttoken
				util.log(`AJAX-request to ${params.response.url} returned (error-)status code ${params.response.status}`);
				if(!this._isErrorStatusCode(tab.prerender.statusCode)) {
					tab.Runtime.evaluate({
						expression: 'window.prerenderReady = true'
					});
				}
				tab.prerender.statusCode = params.response.status;
			}
...
chrome._isErrorStatusCode = function(statusCode) {
	return statusCode && statusCode >= 400 && statusCode <= 599;
}

chrome._resetPageLoadInfo = function(tab) {
	...
	return new Promise((resolve, reject) => {
		...
		Page.enable()
			.then(() => {
				...
				this._navigate(tab, tab.prerender.url).then((result) => {
					...
					if (tab.prerender.navigateError && tab.prerender.navigateError !== 'net::ERR_ABORTED') {
						...
						tab.Page.stopLoading().catch(e => {}); // ignore errors as stopLoading can throw an error if e.g. the connection has been refused and therefore the page isn´t loading anymore
					}
...
chrome._navigate = async function(tab, urlToOpen) {
	let history = await tab.Page.getNavigationHistory();
	if(history && history.entries && tab.prerender.statusCode == 200) { // only switch page if last statuscode was 200, otherwise do a real browser-navigation to the desired page
		let currentPage = history.entries[history.currentIndex];
		let parsedUrlToOpen = url.parse(urlToOpen);
		if(currentPage && currentPage.url && url.parse(currentPage.url).host == parsedUrlToOpen.host && parsedUrlToOpen.pathname) {
			util.log(`Switching to ${urlToOpen} (tabId: ${tab.target})`);
			await tab.Runtime.evaluate({
				expression: 'window.prerenderReady = false;' +
					'var templateDiv = document.getElementById("template");' +
					'var linkToClick = document.createElement("a");' +
					'linkToClick.href = "' + parsedUrlToOpen.pathname + '";' +
					'linkToClick.style.display = "none";' +
					'templateDiv.appendChild(linkToClick);' +
					'linkToClick.click();'
			});
			tab.prerender.domContentEventFired = true;
			tab.prerender.pageLoadInfo.domContentEventFiredMs = 0;
			return {};
		}
	}
	util.log(`Opening ${urlToOpen} (tabId: ${tab.target})`);
	tab.prerender.initialRequestId = undefined;
	tab.prerender.statusCode = undefined;
	return tab.Page.navigate({
		url: urlToOpen
	})
}
...
chrome.checkIfPageIsDoneLoading = function (tab) {
	return new Promise((resolve, reject) => {
		...
		tab.Runtime.evaluate({
			...
			const prerenderReadyDelay = tab.prerender.prerenderReadyDelay || 0;
