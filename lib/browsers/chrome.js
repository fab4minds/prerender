...
const {first, mapTo, merge, BehaviorSubject, filter} = require("rxjs");
...
chrome._getCurrentlyUnusedTab = function() {
	let tab = this.tabs.find(tab => !tab.prerender || tab.prerender.responseSent);
	if(tab) {
		tab.prerender.responseSent = false;
	}
	return tab;
...
chrome._initTabQueue = async function() {
	if(!this._initializedQueue) {
		...
		let promises = new Array(this.options.numTabs).fill(null).map(() => {
			return this.getNewTab().then(tab => this.tabs.push(tab));
		});
		return Promise.all(promises).then(() => {
			merge(...this.tabs.map(tab => tab.prerender.o_responseSent.pipe(filter(responseSent => responseSent), mapTo(tab)))).subscribe(unusedTab => {
