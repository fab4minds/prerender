...
chrome.openTab = function (options) {
	return new Promise((resolve, reject) => {
		...
		this.getUnusedOrNewTab().then((tab) => {
...
chrome.getUnusedOrNewTab = async function() {
	let tab = this._getCurrentlyUnusedTab();
	if(!tab) {
		if(this.tabs.length < 5) { // TODO: replace with constant
			tab = await this.getNewTab();
			this.tabs.push(tab);
		} else {
			tab = await this._getUnusedTab();
		}
	}
	return tab;
}
...
chrome._getCurrentlyUnusedTab = function() {
	let tab = this.tabs.find(tab => tab && tab.prerender && tab.prerender.responseSent);
	tab.responseSent = false;
	return tab;
}

chrome._getUnusedTab = async function() {
	let unusedTab = this._getCurrentlyUnusedTab();
	if(!unusedTab) {

	}
	unusedTab.responseSent = false;
	return unusedTab;
}

chrome._initTabQueue = function() {
