...
const PAGE_DONE_CHECK_INTERVAL = process.env.PAGE_DONE_CHECK_INTERVAL || 200;
...
const AMOUNT_TABS = process.env.AMOUNT_TABS || 5;

const BASE_URL = process.env.BASE_URL;

const AMOUNT_REQUESTS_AUTO_RESTART = process.env.AMOUNT_REQUESTS_AUTO_RESTART || 5; // restart the tab after x requests
...
server.init = function(options) {
	...
	this.options.numTabs = this.options.numTabs || AMOUNT_TABS;
	this.options.baseUrl = this.options.baseUrl || BASE_URL;
	this.options.numRequestsAutoRestart = this.options.numRequestsAutoRestart || AMOUNT_REQUESTS_AUTO_RESTART;
...
server.onRequest = function(req, res) {
	...
	.then(() => {
		...
		setTimeout(() => {
			if (!req.prerender.tab || !req.prerender.tab.prerender.responseSent) {
...
server.finish = function(req, res) {
	...
	if(req.prerender.tab) {
		req.prerender.tab.prerender.responseSent = true;
	}
	this.removeRequestFromInFlight(req);
...
server._send = function(req, res) {
	...
	let tabId;
	if(req.prerender.tab) {
		tabId = req.prerender.tab.target;
	}
	util.log('got', req.prerender.statusCode, 'in', ms + 'ms', 'for', req.prerender.url, '(tabId: ' + tabId + ')');
};
